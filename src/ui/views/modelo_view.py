...
class ModeloView(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.kpi_modelos = kpi_card(kpi_row, "Modelos carregados", "0", "primary")
        self.kpi_modelos.grid(row=0, column=0, sticky="ew", padx=4)
        self.kpi_placeholders = kpi_card(kpi_row, "Placeholders únicos", "0", "accent")
        self.kpi_placeholders.grid(row=0, column=1, sticky="ew", padx=4)
        self.kpi_mapeados = kpi_card(kpi_row, "Mapeados", "0/0", "success")
        self.kpi_mapeados.grid(row=0, column=2, sticky="ew", padx=4)

        # Drop zone + botões
        drop = card(self)
        drop.pack(fill="x", padx=20, pady=10)
        ctk.CTkLabel(drop, text="Arraste e solte aqui ou clique para selecionar", font=FONTS["body"], text_color=c["text_muted"]).pack(pady=(16,4))
